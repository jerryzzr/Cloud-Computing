...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        print("Received "+str(assignment['StudentID'])+","+str(assignment['Module'])+","+str(assignment['status']))
        if(assignment['status']=='submitted'):
            assignment['status']='corrected'
            channel.basic_publish(exchange='assignment_exchange',routing_key='Validation_queue',body=json.dumps(assignment))
        #else:
        #    channel.basic_publish(exchange='assignment_exchange',routing_key='',body=json.dumps(assignment))
    
    channel.basic_consume(queue='Cloud_computing_queue', on_message_callback=callback, auto_ack=False)
